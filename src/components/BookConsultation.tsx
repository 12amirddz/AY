...
import { Stethoscope, ShieldCheck, User, Calendar, MessageSquare, CreditCard, Clock } from 'lucide-react';
...
export default function BookConsultation({ lang, theme, onBack }: BookConsultationProps) {
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 ${cardBg} p-6 lg:p-8 rounded-[2rem] border shadow-sm`}>
        <div>
          <h2 className="text-3xl font-black mb-2">{t.consultations.title}</h2>
          <div className="flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400">
            <span onClick={onBack} className="text-emerald-600 hover:text-emerald-700 cursor-pointer">{t.nav.home}</span>
            <span>/</span>
            <span className="text-emerald-600 hover:text-emerald-700 cursor-pointer">{t.nav.consultations}</span>
            <span>/</span>
            <span className="text-slate-400">{t.consultations.title}</span>
          </div>
        </div>
        <div className="w-16 h-16 bg-[#e8f6f3] dark:bg-emerald-900/20 rounded-2xl flex items-center justify-center shrink-0">
          <Stethoscope className="w-8 h-8 text-[#117676] dark:text-emerald-400" />
        </div>
      </div>
        ...
        <div className={`lg:w-2/3 ${cardBg} p-6 sm:p-8 lg:p-10 rounded-[2.5rem] border shadow-[0_8px_30px_rgb(0,0,0,0.04)]`}>
          <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="pt-4">
              ...
              >
                {isAR ? 'بدء الاستشارة' : 'Start Consultation'}
